...
export default function FormInput({
  ...
  return (
    <label className="w-full block">
      {label && (
        <span className="block font-small mb-2 text-text">{label}</span>
      )}
      <div className="relative">
        {Icon && <Icon className="absolute bottom-4 left-3 text-subText" />}
        <input
          id={id}
          ref={ref}
          maxLength={maxLength}
          ...
          className={`${className} w-full outline-none bg-dashboard ${
            Icon ? "pl-9" : "pl-4"
